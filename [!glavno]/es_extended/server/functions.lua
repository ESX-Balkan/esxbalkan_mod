...
ESX.RegisterCommand = function(name, group, cb, allowConsole, suggestion)
	...
	RegisterCommand(name, function(playerId, args, rawCommand)
		...
		if not command.allowConsole and playerId == 0 then
			print(('[^1UPOZORENJE^7] ^5%s'):format(_U('commanderror_console')))
		else
			...
			if error then
				if playerId == 0 then
					print(('[^1UPOZORENJE^7] %s^7'):format(error))
				else
				...
				cb(xPlayer or false, args, function(msg)
					if playerId == 0 then
						print(('[^1UPOZORENJE^7] %s^7'):format(msg))
					else
...
ESX.TriggerServerCallback = function(name, requestId, source, cb, ...)
	if ESX.ServerCallbacks[name] then
		...
		print(('[^1UPOZORENJE^7] Server callback ^5"%s"^0 ne postoji. ^1Provjerite skripte još jednom!^0'):format(name))
	end
end
...
ESX.SavePlayers = function(cb)
	...
	if #xPlayers > 0 then
		...
		function(result)
			if result then
				if cb then cb() else print(('[^5SERVER^7] Sačuvano %s od %s igrač(a) preko %s sekundi^0'):format(result.affectedRows, #xPlayers, os.time() - time)) end
			end
		end)
	end
end
