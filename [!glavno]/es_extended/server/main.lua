...
CreateThread(function()
	SetMapName('ESX Balkan')
	SetGameType('ESX Balkan Legacy')
...
function loadESXPlayer(identifier, playerId, isNew)
	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll(LoadPlayer, { identifier
			...
			if ESX.DoesJobExist(job, grade) then
				...
				print(('[^1UPOZORENJE^7] Ignorisanje nepostojeceg posla za %s [posao: %s, rank: %s]'):format(identifier, job, grade))
			...
			if result[1].inventory and result[1].inventory ~= '' then
				...
				for name,count in pairs(inventory) do
					...
					if item then
						...
						print(('[^1UPOZORENJE^7] Ignorisanje nepostojeceg itema "%s" za "%s"'):format(name, identifier))
					end
				end
			end
	...
	Async.parallel(tasks, function(results)
		...
		print(('[^5SERVER^0] Igrač ^5"%s" ^0se povezao na server!. ID: ^5%s^7'):format(GetPlayerName(xPlayer.source), playerId))
	end)
end
...
AddEventHandler('chatMessage', function(playerId, author, message)
	if message:sub(1, 1) == '/' and playerId > 0 then
		...
		TriggerClientEvent('esx:showNotification', playerId, ('['..commandName..'] Nije tačna komanda!'))
	end
end)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sendToDiscord('Davanje Itema', GetPlayerName(source) .. ' je dao ' .. ' ' .. itemName .. ' ' .. ' kolicina je ' .. itemCount .. ' igracu ' .. GetPlayerName(target))
			else
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
            ...
            sendToDiscord('Davanje Para', GetPlayerName(source) .. ' je dao ' .. ' ' .. itemName .. ' ' .. ' kolicina je ' .. itemCount .. ' igracu ' .. GetPlayerName(target))
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo and itemCount > 0 then
					...
					sendToDiscord('Davanje oruzija', GetPlayerName(source) .. ' je dao ' .. ' ' .. ESX.GetWeaponLabel(weaponName) .. ' igracu ' .. GetPlayerName(target))
				else
...
AddEventHandler('esx:useItem', function(itemName)
    ...
    local canUse = xPlayer.getInventoryItem(itemName).usable
