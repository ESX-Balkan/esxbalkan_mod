...
window.onload = function () {
				...
				if (data.status == true) {
					if (vehicleInfo.classList.contains('inactive')) {
						...
						if (vehiclesCars.indexOf(data.type) > -1) {
							...
							document.querySelector('#vehicle-gear').style.display = 'inline-block';

							vehicleSeatbelt.style.display = 'inline-block';
							vehicleLights.style.display = 'inline-block';
							vehicleFuel.style.display = 'inline-block';
							
							document.querySelector('#vehicle-gear').style.display = 'inline-block';
						}

						else {
							document.querySelector('#vehicle-others').style.display = 'inline-block';
					...
					if ( (data.seatbelt.status == true) && (vehicleSeatbelt.classList.contains('on') == false) ) {
						...
						eventCallback.sound('sounds/seatbelt-buckle.ogg', { volume: '0.30' });
					}
						...
						eventCallback.sound('sounds/seatbelt-unbuckle.ogg', { volume: '0.30' });
					}
		...
		window.addEventListener('message', function(event) {
			// if (event.data.action != "toggleUi") console.log(event.data.action);
