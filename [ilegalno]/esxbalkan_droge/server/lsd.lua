...
RegisterServerEvent('esxbalkan_droge:processLSD')
AddEventHandler('esxbalkan_droge:processLSD', function()
	if not playersProcessingLSD[source] then
		...
		print(('esxbalkan_droge: %s attempted to exploit lsd processing!'):format(GetPlayerIdentifiers(source)[1]))
	end
end)

RegisterServerEvent('esxbalkan_droge:processThionylChloride')
AddEventHandler('esxbalkan_droge:processThionylChloride', function()
	if not playersProcessingLSD[source] then
		local _source = source
