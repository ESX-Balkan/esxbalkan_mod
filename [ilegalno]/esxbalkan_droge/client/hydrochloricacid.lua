...
CreateThread(function()
	while true do
		...
		if #(coords - Config.CircleZones.HydrochloricAcidFarm.coords) < 50 then
...
Citizen.CreateThread(function()
	while true do
		...
		for i=1, #HydrochloricAcidBarrels, 1 do
			if #(coords - GetEntityCoords(HydrochloricAcidBarrels[i])) < 1 then
			...
			if IsControlJustReleased(0, Keys['E']) and not isPickingUp then
				...
				ESX.TriggerServerCallback('esxbalkan_droge:canPickUp', function(canPickUp)
					...
					if canPickUp then
						...
						TriggerServerEvent('esxbalkan_droge:pickedUpHydrochloricAcid')
					else
