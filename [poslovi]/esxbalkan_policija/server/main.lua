...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if itemType == 'item_standard' then
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceXPlayer.canCarryItem(itemName, sourceItem.count) then
				...
				posaljiPolicija('Oduzeti Item', GetPlayerName(source) ..' je oduzeo stvar: '.. sourceItem.label.. ' od igraca ' ..GetPlayerName(target).. ' kolicine: ' ..amount)
			else
	...
	elseif itemType == 'item_weapon' then
		...
		if targetXPlayer.hasWeapon(itemName) then
			...
			posaljiPolicija('Oduzeti prljav novac', GetPlayerName(source) ..' je oduzeo prljav novac kolicine: $'.. amount ..' od igraca: ' ..GetPlayerName(target))
		else
			sourceXPlayer.showNotification(_U('quantity_invalid'))
		end
	end
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if count > 0 and inventoryItem.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				posaljiPolicija('Uzimanje Itema', GetPlayerName(source) .. ' Je izvadio ' .. inventoryItem.name .. ' ' .. count)
			else
...
AddEventHandler('esx_policejob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_police', function(inventory)
		...
		if sourceItem.count >= count and count > 0 then
			...
			posaljiPolicija('Stavljanje Itema', GetPlayerName(source) .. ' Je Stavio ' .. inventoryItem.name .. ' ' .. count)
		else
			xPlayer.showNotification(_U('quantity_invalid'))
		end
	end)
...
ESX.RegisterServerCallback('esx_policejob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	...
	if removeWeapon then
		...
		posaljiPolicija("Stavljanje oruzija", GetPlayerName(source).. " ".. "je ostavio".. " ".. ESX.GetWeaponLabel(weaponName).. " sa 100 metaka".. " u sef")
	end
...
ESX.RegisterServerCallback('esx_policejob:removeArmoryWeapon', function(source, cb, weaponName)
    ...
    posaljiPolicija("Vadjenje oruzija", GetPlayerName(source).. " ".. "je izvadio".. " ".. ESX.GetWeaponLabel(weaponName).. " sa 250 metaka".. " iz sef")
	TriggerEvent('esx_datastore:getSharedDataStore', 'society_police', function(store)
		local weapons = store.get('weapons') or {}
	...
	if not selectedWeapon then
		...
		if type == 1 then
			if xPlayer.getMoney() >= selectedWeapon.price then
                ...
                posaljiPolicija('Kupovina Oruzija', GetPlayerName(source) .. ' je kupio ' .. ESX.GetWeaponLabel(weaponName) .. ' ' .. ' za '.. selectedWeapon.price)
				cb(true)
			else
				cb(false)
			end
		...
		elseif type == 2 then
			...
			if component then
				if xPlayer.getMoney() >= price then
                    ...
                    posaljiPolicija('Kupovina Komponenata', GetPlayerName(source) .. ' je kupio ' .. component.name .. ' ' .. ' za ' .. ESX.GetWeaponLabel(weaponName))
					cb(true)
				else
					cb(false)
				end
...
function posaljiPolicija(name,message)
	local embeds = {{
		["title"]=message,
		["type"]="rich",
		["color"] =2061822,
