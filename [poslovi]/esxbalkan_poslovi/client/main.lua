...
TriggerEvent('esx:getSharedObject', function(obj) ESX = obj end)
  ...
  AddEventHandler('esxbalkan_poslovi:action', function(job, zone, zoneKey)
	  menuIsShowed = true
	  ...
	  elseif zone.Type == "work" then
		  ...
		  if IsPedInAnyVehicle(playerPed, false) then
			  ...
			  TriggerServerEvent('esxbalkan_poslovi:startWork', zone.Item, zoneKey)
		  end
							  ...
							  if playerPed == driverPed then
								  ...
								  for i=1, #myPlate, 1 do
									  if myPlate[i] == plate then
										  ...
										  TriggerServerEvent('esxbalkan_poslovi:caution', "give_back", giveBack, 0, 0)
  ...
  AddEventHandler('esxbalkan_poslovi:hasExitedMarker', function(zone)
	  TriggerServerEvent('esxbalkan_poslovi:stopWork')
	  hintToDisplay = "no hint to display"
	  menuIsShowed = false
	  hintIsShowed = false
	  isInMarker = false
  ...
  function spawnVehicle(spawnPoint, vehicle, vehicleCaution)
	  hintToDisplay = 'no hint to display'
	  hintIsShowed = false
	  TriggerServerEvent('esxbalkan_poslovi:caution', 'take', vehicleCaution, spawnPoint, vehicle)
  end
  
  RegisterNetEvent('esxbalkan_poslovi:spawnJobVehicle')
  AddEventHandler('esxbalkan_poslovi:spawnJobVehicle', function(spawnPoint, vehicle)
	  local playerPed = PlayerPedId()
			  ...
			  if zones ~= nil then
				  ...
				  if IsControlJustReleased(0, Keys['E']) and not menuIsShowed and isInMarker then
					  if onDuty or zone.Type == "cloakroom" or ESX.PlayerData.job.name == "reporter" then
						  TriggerEvent('esxbalkan_poslovi:action', job, zone, currentZone)
					  end
				  end
				  ...
				  if not isInMarker and hasAlreadyEnteredMarker then
					  ...
					  TriggerEvent('esxbalkan_poslovi:hasExitedMarker', zone)
				  end
			  end
		  end
	  end
