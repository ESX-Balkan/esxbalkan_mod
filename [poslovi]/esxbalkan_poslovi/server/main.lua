...
RegisterServerEvent('esxbalkan_poslovi:startWork')
AddEventHandler('esxbalkan_poslovi:startWork', function(zoneIndex, zoneKey)
...
RegisterServerEvent('esxbalkan_poslovi:stopWork')
AddEventHandler('esxbalkan_poslovi:stopWork', function()
	if playersWorking[source] then
		playersWorking[source] = nil
	end
end)

RegisterNetEvent('esxbalkan_poslovi:caution')
AddEventHandler('esxbalkan_poslovi:caution', function(cautionType, cautionAmount, spawnPoint, vehicle)
	local xPlayer = ESX.GetPlayerFromId(source)
		...
		if cautionAmount <= Config.MaxCaution and cautionAmount >= 0 then
			TriggerEvent('esx_addonaccount:getAccount', 'caution', xPlayer.identifier, function(account)
				if xPlayer.getAccount('bank').money >= cautionAmount then
					...
					TriggerClientEvent('esxbalkan_poslovi:spawnJobVehicle', xPlayer.source, spawnPoint, vehicle)
				else
