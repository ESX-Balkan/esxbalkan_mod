	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_mechanic', function(inventory)
		...
		if count > 0 and item.count >= count then
			...
			if xPlayer.canCarryItem(itemName, count) then
				...
				posaljiMehanicar('Uzimanje Itema', GetPlayerName(source) .. ' Je izvadio ' .. item.label .. ' ' .. count)
			else
...
AddEventHandler('esx_mechanicjob:putStockItems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_mechanic', function(inventory)
		...
		posaljiMehanicar('Stavljanje Itema', GetPlayerName(source) .. ' Je Stavio ' .. item.label .. ' ' .. count)
	end)
end)
...
function posaljiMehanicar(name,message)
	local embeds = {{
		["title"]=message,
		["type"]="rich",
		["color"] =2061822,
