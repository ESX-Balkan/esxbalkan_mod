...
window.addEventListener("message", function (event) {
    ...
    //    $("#noSecondInventoryMessage").html(invLocale.secondInventoryNotAvailable);
        ...
        $.post("http://esxbalkan_inventory/GiveItem", JSON.stringify({
            player: player,
...
function closeInventory() {
    $(".ui").fadeOut();
    $.post("http://esxbalkan_inventory/NUIFocusOff", JSON.stringify({}));
	...
	$('#itemFast-1').droppable({
        drop: function (event, ui) {
            ...
            if (type === "normal" && (itemInventory === "main" || itemInventory === "fast")) {
                $.post("http://esxbalkan_inventory/PutIntoFast", JSON.stringify({
                    item: itemData,
                    slot : 1,
                }));
            }
...
$(document).ready(function () {
        ...
        if (type === "normal" && itemInventory === "main" && itemData.usable) {
            if (itemData.name.includes("WEAPON_")) {
                if (itemInventory === "fast") 
                {
                    ...
                    $.post("http://esxbalkan_inventory/TakeFromFast", JSON.stringify({
                        item: itemData
                    }));
                }
                
            } else {
                $.post("http://esxbalkan_inventory/UseItem", JSON.stringify({
                    item: itemData
                }));
                closeInventory();
            }
    ...
    $(document).on('contextmenu', '.item', function (e) {
        ...
        if(e.shiftKey) {
            if (itemInventory === "second") {
                if (type === "trunk") {
                    $.post("http://esxbalkan_inventory/TakeFromTrunk", JSON.stringify({
    
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("http://esxbalkan_inventory/TakeFromProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("http://esxbalkan_inventory/TakeFromVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("http://esxbalkan_inventory/TakeFromPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                    ...
                    $.post("http://esxbalkan_inventory/TakeFromShop", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motels") {
                    $.post("http://esxbalkan_inventory/TakeFromMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
    
                } else if (type === "motelsbed") {
                    $.post("http://esxbalkan_inventory/TakeFromMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("http://esxbalkan_inventory/TakeFromGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
                ...
                if (type === "trunk") {
                    $.post("http://esxbalkan_inventory/PutIntoTrunk", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "property") {
                    $.post("http://esxbalkan_inventory/PutIntoProperty", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val()),
                        owner : ownerHouse
                    }));
                } else if (type === "vault") {
                    $.post("http://esxbalkan_inventory/PutIntoVault", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "player") {
                    $.post("http://esxbalkan_inventory/PutIntoPlayer", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "motels") {
                    $.post("http://esxbalkan_inventory/PutIntoMotel", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }))
                } else if (type === "motelsbed") {
                    $.post("http://esxbalkan_inventory/PutIntoMotelBed", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                } else if (type === "glovebox") {
                    $.post("http://esxbalkan_inventory/PutIntoGlovebox", JSON.stringify({
                        item: itemData,
                        number: parseInt($("#count").val())
                    }));
                }
    ...
    $('#drop').droppable({
        ...
        drop: function (event, ui) {
            ...
            $.post("http://esxbalkan_inventory/DropItem", JSON.stringify({
