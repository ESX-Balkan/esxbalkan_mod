...
AddEventHandler('conde-inventoryhud:useWeapon', function(weapon)
    ...
    TriggerEvent('conde-inventoryhud:notification', weapon,"Oruzije", 1, false)
...
AddEventHandler('tqrp_inventoryhud:useAttach', function(attach)
    ...
    if currentWeapon ~= nil then
		...
		if weapons[tostring( hash )] ~= nil and weapons[tostring( hash )][attach] ~= nil and not hasAttach then
            ...
            exports['mythic_notify']:DoHudText('error', 'Ovaj prilog nije kompatibilan ili je već opremljen.')
        end
    else
        exports['mythic_notify']:DoHudText('error', 'Nije odabrano oružje.')
    end
end)
...
RegisterCommand("desequipar", function(source, args, rawCommand)
    if currentWeapon ~= nil then
                ...
                if currentWepAttachs[i] == attach then
                    ESX.TriggerServerCallback('tqrp_inventoryhud:addPlayerItem', function(cb)
                        if cb then
                            ...
                            exports['mythic_notify']:DoHudText('error', 'Nedovoljno prostora.')
                        end          
                    end, currentWepAttachs[i], 1)
                    return
                end
            end
            exports['mythic_notify']:DoHudText('error', 'Ovo oružje nema ovaj dodatak.')
		else
			for i = 1, #currentWepAttachs do
        ...
        exports['mythic_notify']:DoHudText('error', 'Nemate oružje u ruci.')
    end
end)
