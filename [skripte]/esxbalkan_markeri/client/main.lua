...
function spawnajPropove()
	while spawnaniPropovi < actionsRemaining do
		...
		for i=1, #availableActions do
			...
			ESX.Game.SpawnLocalObject('prop_beach_punchbag', kordinataPropa, function(objekat)
				PlaceObjectOnGroundProperly(objekat)
				FreezeEntityPosition(objekat, true)
	
				table.insert(propTabela, objekat)
				spawnaniPropovi = spawnaniPropovi + 1
			end)
...
function zavrsiMarker(action)
	for i=1, #propTabela, actionsRemaining do
		local action_pos = -1
		for k = 1, #availableActions do
			if action.coords.x == availableActions[k].coords.x and action.coords.y == availableActions[k].coords.y and action.coords.z == availableActions[k].coords.z then
				action_pos = i
				ESX.Game.DeleteObject(propTabela[k])
