    ...
    if type == "item_weapon" then
      TriggerEvent(
        ...
        function(store)
          ...
          --sendToDiscord("Vadjenje Pusaka", xPlayer.name .. " je izvadio " .. ESX.GetWeaponLabel(weaponName) .. " sa " .. ammo .. " metaka " .. " auto je sa tablicama " .. plate)
          local blackMoney = 0
		  local cashMoney = 0
          local items = {}
          local weapons = {}
...
AddEventHandler(
  ...
  function(plate, type, item, count, max, owned, label)
    ...
    if item == "money" then 
      TriggerClientEvent('esx:showNotification', _source, 'Ne možeš ostavljati novac u gepek!')
      return  
     end
	...
	if type == "item_money" then
      local playerAccountMoney = xPlayer.getAccount('money')
    ...
    if type == "item_weapon" then
      TriggerEvent(
        ...
        TriggerClientEvent('esx:showNotification', _source, _U("invalid_amount"))
            ...
            --sendToDiscord("Stavljanje Pusaka", GetPlayerName(source ).. " stavlja " .. ESX.GetWeaponLabel(item) .. " u gepek " .. " auto je sa tablicama " .. plate)		
            MySQL.Async.execute(
              "UPDATE trunk_inventory SET owned = @owned WHERE plate = @plate",
              {
                ["@plate"] = plate,
...
ESX.RegisterServerCallback(
  ...
  function(source, cb)
	...
	local cashMoney = xPlayer.getAccount('money')
